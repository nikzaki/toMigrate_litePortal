...
export class CompetitionService {
  ...
  public getCompetitionTeams(competitionId: number): Observable<CompetitionTeams> {
    ...
    return this.remoteHttp.execute(req).map((resp: Response) => {
  ...
  public getFlightStatus(
    competitionId: number,
    roundNo: number
  ): Observable<CompetitionFlightStatus[]> {
   ...
   */
  public searchUpcomingCompetitions(
    searchString: string,
    pageNo: number,
    pageSize: number = 30,
    organizerId?: number
  ): Observable<CompetitionList> {
  ...
  private _searchCompetitions(criteria: any): Observable<CompetitionList> {
    ...
    let url = this.configService.getRestApiUrl(RestUrl.competitionService.search);
    let request = new RemoteRequest(
      url,
      RequestMethod.Get,
      ContentType.URL_ENCODED_FORM_DATA,
      criteria
    );
    return this.remoteHttp.execute(request).map((resp: Response) => {
      let compList: CompetitionList = resp.json();
      if (compList && compList.competitions)
        compList.competitions.forEach((compInfo: Competition) => {
          this.configService.deriveFulImageURL(compInfo, ['thumbnail', 'imageUrl']);
  ...
  public getTeamEventScores(competitionId: number, grossOrNet?: string): Observable<TeamScores[]> {
  ...
  public mergeObjects(objects: Array<any>): any {
    ...
    if (isPresent(objects && objects.length)) {
      for (let src of objects) {
        if (isPresent(src)) this.mergeObject(result, src);
      }
    }
    return result;
  }
  ...
  public overrideWinners(
    competitionId: number,
    grossOrNet: string = 'G',
    winners: Array<number>,
    category: number,
    updatePrize: boolean = true,
    authToken: string
  ): Observable<any> {
  ...
  public getCompNonPlayedHoles() {
    let _randomizer = Math.floor(Math.random() * 7777) + 1;

    // let _url = 'https://app.mygolf2u.com/settings/comp_nonHolesPlayed.json' + '?v=' + _randomizer;
    let _url = 'config/comp_nonHolesPlayed.json' + '?v=' + _randomizer;

    let req = new RemoteRequest(_url, RequestMethod.Get);
    return this.remoteHttp.execute(req).map((resp: Response) => {
