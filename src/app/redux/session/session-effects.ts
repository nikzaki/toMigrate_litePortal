...
export class SessionEffects {
  ...
  login$ = this.actions$
    ...
    .switchMap((credentials: Credentials) => {
      ...
      return this.authService
        ...
        .catch((error: any) => {
          const msg = Util.getErrorMessage(
            error,
            'Unknown error occured while signing in. Server may be down.'
          );
    ...
    .do((session: Session) => {
      return this.organizerService
        ...
        .subscribe((organizer: Organizer) => {
          ...
          if (session.returnUrl) this.router.navigate([session.returnUrl]);
          else if (_isScorer >= 0 && session.userInfo.organizerId) {
            let _search = '';
            this.organizerService
              .getActiveCompetitions(session.userInfo.organizerId)
              .subscribe((activeComps: Competition[]) => {
                if (activeComps) {
                  if (activeComps.length === 1) {
                    this.router.navigate([
                      '/admin/competition/manualscoring/' + activeComps[0].competitionId,
                    ]);
                  } else {
  ...
  @Effect({ dispatch: false })
  loggedOut$ = this.actions$.ofType(SessionActions.LOGOUT).do((action: Action) => {
