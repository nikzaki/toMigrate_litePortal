...
export class IndividualLeaderboardComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    this.scoringNotifications = this.notfService
    ...
    this.playersToDisplay.sort((a, b) => {
      ...
      if (this.compDetails.roundInProgress) {
        if (!a.position && Number(b.position)) return 1;
          ...
          // else if(a.round4Gross < b.round4Gross) return -1;
          else {
  ...
  private refreshCompInfo() {
    ...
    if (this.competitionId >= 0) {
      let sub1 = this.competitionService
      ...
      let sub3 = this.competitionService
        .getCompetitionDataLite(this.competitionId)
  ...
  subGetLeaderboard: Subscription;
  refreshLeaderBoard() {
    ...
    if (this.refreshParams) {
      ...
      const subLeaderboard = this.competitionService.getNewLeaderboard(
        this.competitionId,
  ...
  isSponsorHidden() {
    ...
    if (!this.settings.showSponsor) return true;
    // && this.compDetails.sponsors.length > 0
    else if (this.settings.showSponsor) return false;
  }
      ...
      // return ''; //_time
        ...
        if (this.settings.selectedRound === 0) {
          ...
          return (
            ...
            (18 * this.compDetails.roundInProgress - Number(data.thru))
          );
      }
      // return 18-(18*this.compDetails.roundInProgress - Number(data.thru))
  ...
  goTeamLeaderboard() {
    this.router.navigateByUrl(
      `teamleaderboard/${this.competition.competitionId}?showTeamOnly=true`
    );
  }
